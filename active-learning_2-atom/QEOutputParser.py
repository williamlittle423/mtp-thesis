...
class QEOutputParser:
    def parse_qe_output(self, qe_output):
        ...
        for i, line in enumerate(lines):
            # Parse lattice parameter (alat) in Bohr or Å
            if "lattice parameter (alat)" in line:
                value = float(line.split("=")[-1].strip().split()[0])
                # If value > 4, assume it's in Bohr; otherwise, assume it's already in Å
                if value > 4.0:
                    alat = value * 0.529177  # Convert Bohr to Å
                else:
                    alat = value
            # Parse cell vectors
            ...
            # Parse atomic positions (fractional coordinates)
                ...
                atom_positions.append([float(x) * alat for x in position_values])  # Convert Bohr → Å if needed
                num_atoms += 1  # Count atoms
            # Parse forces (Ry/Bohr → eV/Å)
            elif "force =" in line:
                forces.append([float(x) * 25.711 for x in line.split("=")[-1].strip().split()])
            ...
            # Parse total energy 
                ...
                energy = ((float(line.split("=")[-1].strip().split()[0]) * 13.6057)) / num_atoms
    ...
    def append_mtp_configurations(self, qe_output_files, mtp_config_file):
        ...
        if os.path.exists(mtp_config_file):
        ...
        with open(mtp_config_file, "a") as cfg_file:
            for qe_output_file in qe_output_files:
                ...
                try:
                    ...
                    # Append only if unique, and add an extra newline before appending
