...
for i in range(num_configurations):
    ...
    perturbed_celldm = equilibrium_celldm * (1 + np.random.uniform(-perturbation_scale, perturbation_scale))

    # Apply random perturbation to atomic positions
    perturbed_positions = base_positions + np.random.uniform(-perturbation_scale, perturbation_scale, base_positions.shape)
